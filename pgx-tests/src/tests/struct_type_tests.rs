...
fn complex_in(input: &std::ffi::CStr) -> PgBox<Complex> {
    ...
    (&mut complex).x = str::parse::<f64>(&x).expect(&format!("{} isn't a f64", x));
    (&mut complex).y = str::parse::<f64>(&y).expect(&format!("{} isn't a f64", y));
    ...
    fn test_complex_storage_and_retrieval() {
        let complex = Spi::connect(|mut client| {
            Ok(client.update(
                ...
                SELECT value FROM complex_test ORDER BY id;", None, None).get_one::<PgBox<Complex>>())
        });
