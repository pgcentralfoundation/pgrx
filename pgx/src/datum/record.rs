...
impl FromDatum for Record {
    const GET_TYPOID: bool = true;

    #[inline]
    unsafe fn from_datum(_datum: pg_sys::Datum, _is_null: bool) -> Option<Record> {
    ...
    unsafe fn from_polymorphic_datum(
        ...
        if is_null {
            ...
            Some(Record { datum, typoid })
        }
    }
}

impl IntoDatum for Record {
    #[inline]
...
unsafe impl SqlTranslatable for Record {
    fn argument_sql() -> Result<SqlMapping, ArgumentError> {
        Ok(SqlMapping::literal("record"))
    }
    fn return_sql() -> Result<Returns, ReturnsError> {
        Ok(Returns::One(SqlMapping::literal("record")))
    }
}
