...
use petgraph::{stable_graph::{NodeIndex, StableGraph}, dot::Dot, visit::{IntoNodeReferences, IntoNeighborsDirected}, algo::toposort};
...
impl<'a> PgxSql<'a> {
    ...
    pub fn to_dot(&self, file: impl AsRef<str> + Debug) -> eyre::Result<()> {
        ...
        let generated = Dot::with_attr_getters(
            ...
            &|graph, edge| { format!("label = {:?}", edge.weight()) },
            &|graph, (index, node)| { format!("label = {:?}", node.source_path()) },
        );
        let path = Path::new(file.as_ref());
    ...
    pub fn to_sql(&self) -> eyre::Result<String> {
        ...
        for step_id in toposort(&self.graph, None).map_err(|e| eyre_err!("Depgraph was Cyclic."))? {
            ...
            tracing::trace!(?step_id, ?step);
            let item_sql = step.to_sql(&self)?;
            tracing::trace!(sql = ?item_sql);
