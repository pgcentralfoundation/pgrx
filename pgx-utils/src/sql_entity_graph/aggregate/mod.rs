        ...
        let entity_item_fn: ItemFn = parse_quote! {
            ...
            pub extern "C" fn #sql_graph_entity_fn_name() -> *mut u8 {
                let submission = ::pgx::utils::sql_entity_graph::PgAggregateEntity {
                    ...
                    stype: #type_stype,
                    sfunc: stringify!(#fn_state).into(),
                    combinefunc: None #( .unwrap_or(Some(stringify!(#fn_combine_iter).into())) )*,
                    finalfunc: None #( .unwrap_or(Some(stringify!(#fn_finalize_iter).into())) )*,
                    finalfunc_modify: None #( .unwrap_or(#const_finalize_modify_iter) )*,
                    initcond: None #( .unwrap_or(Some(#const_initial_condition_iter.into())) )*,
                    serialfunc: None #( .unwrap_or(Some(stringify!(#fn_serial_iter).into())) )*,
                    deserialfunc: None #( .unwrap_or(Some(stringify!(#fn_deserial_iter).into())) )*,
                    msfunc: None #( .unwrap_or(Some(stringify!(#fn_moving_state_iter).into())) )*,
                    minvfunc: None #( .unwrap_or(Some(stringify!(#fn_moving_state_inverse_iter).into())) )*,
                    mstype: None #( .unwrap_or(Some(#type_moving_state_entity_tokens_iter)) )*,
                    mfinalfunc: None #( .unwrap_or(Some(stringify!(#fn_moving_finalize_iter).into())) )*,
                    mfinalfunc_modify: None #( .unwrap_or(#const_moving_finalize_modify_iter) )*,
                    minitcond: None #( .unwrap_or(Some(#const_moving_intial_condition_iter.into())) )*,
                    sortop: None #( .unwrap_or(Some(#const_sort_operator_iter.into())) )*,
