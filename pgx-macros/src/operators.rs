use pgx_utils::{operator_common::*, sql_entity_graph_generators};
...
pub(crate) fn impl_postgres_ord(ast: DeriveInput) -> syn::Result<proc_macro2::TokenStream> {
    ...
    let sql_graph_entity_item = sql_entity_graph_generators::PostgresOrd::from_derive_input(ast)?;
...
pub(crate) fn impl_postgres_hash(ast: DeriveInput) -> syn::Result<proc_macro2::TokenStream> {
    ...
    let sql_graph_entity_item = sql_entity_graph_generators::PostgresHash::from_derive_input(ast)?;
    sql_graph_entity_item.to_tokens(&mut stream);
