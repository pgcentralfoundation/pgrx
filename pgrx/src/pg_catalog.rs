...
impl<'a> GetStruct<&'a pgrx::pg_sys::nameData> for &'a CStr {
    unsafe fn get_struct(raw: &pgrx::pg_sys::nameData) -> Self {
        assert_eq!(raw.data[63], 0);
        unsafe { CStr::from_ptr(raw.data.as_ptr()) }
    }
}

// A variable-sized type that is always the last struct member in `FromData_*`.
impl<'a> GetStruct<&'a pgrx::pg_sys::int2vector> for &'a [i16] {
    unsafe fn get_struct(raw: &'a pgrx::pg_sys::int2vector) -> Self {
        let len = raw.dim1;
...
// A variable-sized type that is always the last struct member in `FromData_*`.
impl<'a> GetStruct<&'a pgrx::pg_sys::oidvector> for &'a [Oid] {
    unsafe fn get_struct(raw: &'a pgrx::pg_sys::oidvector) -> Self {
        let len = raw.dim1;
        // SAFETY: we trust `len` since it's passed from PostgreSQL and we cannot check it anyway
        unsafe { raw.values.as_slice(len as usize) }
    }
        ...
        paste::paste! {
            ...
            impl pgrx::datum::FromDatum for [<$table:camel $column:camel>] {
                    ...
                    Self: Sized,
                {
                    if is_null {
                        ...
                        Some(Self::new(datum.value() as _))
                    }
                }
            }
            ...
            unsafe impl pgrx::datum::UnboxDatum for [<$table:camel $column:camel>] {
                ...
                where
                    Self: 'src,
                {
                    Self::new(datum.sans_lifetime().value() as c_char)
                }
            }
            ...
            impl [<$table:camel>]<'_> {
                ...
                pub fn $column(&self) -> $column_type {
                    unsafe {
                        ...
                        let offset: u8 = (*self.inner.t_data).t_hoff;
                        // So `s` is deferenced `GETSTRUCT(tup)`.
                        let s = &*start.add(offset as usize).cast::<pgrx::pg_sys::[<FormData_ $table>]>();
        ...
        paste::paste! {
            impl [<$table:camel>]<'_> {
                ...
                pub fn $column(&self) -> Option<$column_type> {
                    self.get_attr::<$column_type>(pgrx::pg_sys::[<Anum_ $table _ $column>])
                }
            }
        }
    };
    {
        ...
        paste::paste! {
            impl [<$table:camel>]<'_> {
                $(#[$column_meta])*
                pub fn $column(&self) -> $column_type {
                    self.get_attr::<$column_type>(pgrx::pg_sys::[<Anum_ $table _ $column>]).unwrap()
                }
            }
        }
    };
        ...
        paste::paste!{
            ...
            impl [<$table:camel SearchList>] {
                ...
                pub fn len(&self) -> usize {
                    unsafe {
                        use pgrx::pg_sys::CatCList;
                        let inner: &CatCList = self.inner.as_ref();
                ...
                pub fn get(&self, i: usize) -> Option<[<$table:camel>]> {
                    unsafe {
                        use pgrx::pg_sys::{CatCList, CatCTup, HeapTupleData};
                        let inner: &CatCList = self.inner.as_ref();
                        let n: i32 = inner.n_members;
        ...
        paste::paste!{
            impl<'a> [<$table:camel>]<'a> {
                ...
                pub fn [<search _ $cache>]($_0: $_0_type) -> Option<[<$table:camel Search>]> {
                    ...
                    let entry = unsafe { pgrx::pg_sys::SearchSysCache1(cache_id, $_0.into_datum()?) };
        ...
        paste::paste!{
            impl<'a> [<$table:camel>]<'a> {
                ...
                pub fn [<search _list_ $cache _1>]($_0: $_0_type) -> Option<[<$table:camel SearchList>]> {
                    ...
                    let cache_id = pgrx::pg_sys::[<SysCacheIdentifier_ $cache:upper>] as i32;
                    let entry = unsafe {
                        pgrx::pg_sys::SearchSysCacheList(
                            cache_id,
                            1,
                            $_0.into_datum()?,
                            0.into(),
