...
**Why `TYPE_IDENT` has no default:** If a trait default used `module_path!()`, it
...
### Meaning of `TYPE_IDENT`

`TYPE_IDENT` is the compile-time dependency identity of a Rust type for schema
generation. It is not the SQL spelling of the type.

- For derive-generated types and enums, `TYPE_IDENT` is canonical and generated
  automatically.
- For aliases and re-exports, the compiler resolves the same underlying
  `SqlTranslatable` impl, so they naturally reuse the same `TYPE_IDENT`.
- For wrapper impls that should order against the underlying SQL type
  (`Option<T>`, `Result<T, E>`, `Vec<T>`, `Array<T>`, `PgBox<T>`, and similar),
  the wrapper delegates its `TYPE_IDENT` to the inner type.
...
`TYPE_IDENT` strings. Authors are expected to treat `TYPE_IDENT` as the
    ...
    TOTAL_LEN,
    {
        let writer =
            ::pgrx::pgrx_sql_entity_graph::section::EntryWriter::<TOTAL_LEN>::new()
                ...
                .bool(false)
                .bool(false)
                .str(<MyType as SqlTranslatable>::TYPE_IDENT)
...
- **Const section encoding** happens in the proc macro through
  `section::EntryWriter::<N>` chains. Type-dependent pieces such as type identity and
  SQL mappings come from generated references to
  `SqlTranslatable::{TYPE_IDENT, TYPE_ORIGIN, ARGUMENT_SQL, RETURN_SQL}`.
...
**Will now generate:**
- `unsafe impl SqlTranslatable for MyType` with `TYPE_IDENT` and const SQL metadata
  set automatically
...
```rust
const _: () = {
    let _ = <MyType as SqlTranslatable>::TYPE_IDENT;
    let _ = <MyType as SqlTranslatable>::ARGUMENT_SQL;
};

const _: () = {
    let _ = <ReturnType as SqlTranslatable>::TYPE_IDENT;
...
module path.
The graph builder connects them to their type entity by `TYPE_IDENT`.
...
The `PgrxSql` graph builder is modified to match entities by `TYPE_IDENT` instead of
`TypeId`.

**Type matching algorithm:**
1. For each type-bearing edge source (function arg, function return, aggregate state
   type, `creates = [Type(T)]`, etc.), read the embedded `TYPE_IDENT`
...
5. Otherwise, fail with an unresolved-type-ident error

For `PostgresOrd`/`PostgresHash`/aggregate connections, the same `TYPE_IDENT` matching
...
| `SqlDeclaredEntity` / `creates` system | Same declaration mechanism, but enriched with `TYPE_IDENT`/SQL metadata for type-bearing declarations |
...
`TYPE_IDENT` alongside the declaration.

Declared type and enum entries stay lean: they carry `TYPE_IDENT` plus the SQL
mapping needed for rendering, but they do not duplicate `TYPE_ORIGIN`.

This is important for manual/custom SQL-backed types:

- dependency edges come from `TYPE_IDENT` matching
...
types used in function signatures add `TYPE_IDENT` plus const SQL metadata. Once they
do, those types work for both arguments and return positions under the single-pass
scheme.

## Type Matching Strategy: Detailed Rules

### 1. Type identity comes from `TYPE_IDENT`
...
`<T as SqlTranslatable>::TYPE_IDENT`.
...
1. Search registered type and enum entities for a matching `TYPE_IDENT`
2. Search `extension_sql!()` `creates` declarations for a matching `TYPE_IDENT`
...
resolves `<MyAlias as SqlTranslatable>::TYPE_IDENT` and the associated SQL metadata
...
`SqlTranslatable` impl, it carries the same `TYPE_IDENT` and the same SQL metadata.
...
instantiations can work if their `SqlTranslatable` impl exposes a stable `TYPE_IDENT`
...
The `TYPE_IDENT` design already handles aliases and re-exports for
...
debugging aid or to validate that a manual `TYPE_IDENT` matches the underlying type the
user intended.
...
local rendering feature layered on top of the `TYPE_IDENT` model, not as a
replacement for it.
...
2. **Add compile-time schema metadata to every manual `unsafe impl SqlTranslatable`
   block.** At minimum that means `TYPE_IDENT`, `TYPE_ORIGIN`, `ARGUMENT_SQL`,
   and `RETURN_SQL`:
   ```rust
   const TYPE_IDENT: &'static str = pgrx::pgrx_resolved_type!(YourType);
   const TYPE_ORIGIN: TypeOrigin = TypeOrigin::ThisExtension;
   const ARGUMENT_SQL: Result<SqlMappingRef, ArgumentError> =
...
1. **`pgrx-sql-entity-graph`** — Each entity module's proc macro expansion changes
   ...
   to `TYPE_IDENT`-based matching and consumes const-friendly SQL metadata.
...
4. **`pgrx`** — The `pgrx_embed!()` macro and `WithTypeIds` infrastructure are removed.
   `SqlTranslatable` gains required const schema metadata (`TYPE_IDENT`,
